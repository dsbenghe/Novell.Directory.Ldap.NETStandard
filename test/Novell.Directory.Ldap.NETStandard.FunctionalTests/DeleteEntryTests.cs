...
namespace Novell.Directory.Ldap.NETStandard.FunctionalTests
{
    public class DeleteEntryTests
    {
        ...
        public async Task Delete_OfExistingEntry_ShouldWork()
        {
            ...
            var retrivedEntry = await LdapOps.GetEntryAsync(existingEntry.Dn);
        ...
        public async Task Delete_OfNotExistingEntry_ShouldThrownNoSuchObject()
        {
            var ldapException = await Assert.ThrowsAsync<LdapException>(
                async () => await TestHelper.WithAuthenticatedLdapConnectionAsync(async ldapConnection => { await ldapConnection.DeleteAsync(TestHelper.BuildDn(Guid.NewGuid().ToString())); }));
